...
## Часть IV. Продвижение (минимальное, без контент-марафонов)

Принцип: код и README делают 90% работы. Постов — три обязательных, дальше только по событиям.

### 4.1. Обязательный минимум

**1. Пост-ставка (до старта, 10 минут).** TG + X, одна мысль в 3–4 предложения: «Компании платят $500–2500/мес за AI-поиск лидов. Строю open-source альтернативу на Go — self-hosted, свои данные, без спам-машины. Прогресс покажу здесь». Публичная ставка создаёт интригу и обязательство; во время разработки посты не обязательны (максимум — скриншот без текста, когда есть что показать).

**2. Пост-развязка (день релиза).** «Обещал — вот оно» + скриншоты UI + цифры, которые агент успел собрать, пока вы его писали (N сигналов, M горячих, 1–2 обезличенных примера). Тот же пост адаптируется в LinkedIn.

**3. День дистрибуции (полдня, один раз).**
- **Show HN** (вт–чт, 14–16 UTC): «Show HN: Self-hosted AI agent that finds leads from intent signals (Go, single binary)». Весь день отвечать в комментариях — скорость реакции решает.
- **Один** сабреддит в тот же день: r/selfhosted (угол: privacy и цена) или r/golang (угол: single binary, embed UI). Второй — через 3–4 дня, не одновременно.
- Хабр-статья — опционально и позже, только если будет желание: «как я строил X» пишется за вечер по горячим следам и приводит русскоязычную волну.

### 4.2. После запуска: только по событиям

Никаких расписаний. Пост пишется за 15 минут, когда случилось что-то показываемое:
- заметный релиз/фича → скриншот + 3 предложения;
- круглые цифры (100/500 звёзд, первые внешние юзеры) → build-in-public пост с выводами;
- агент нашёл лид, который стал деньгами → главный пост-кейс (это конвертирует сильнее всего остального вместе взятого);
- интересный issue или неожиданный use case от пользователя → пост.

Нет событий — нет постов. Проект, который работает, генерирует поводы сам.

### 4.3. KPI (упрощённые)

Единственные три числа, на которые стоит смотреть после запуска: звёзды GitHub (упаковка работает?), внешние пользователи с issues/вопросами (нужен ли продукт?), входящие запросы на консалтинг/помощь (конвертируется ли в деньги?). Если через 2–3 недели все три на нуле — чинить README и заголовки, потом каналы, и только потом думать про продукт.

### 4.4. Анти-чеклист

- Не покупать звёзды/подписчиков — дев-аудитория чует мгновенно.
- Не спамить ссылкой в чужих чатах — только полезный ответ, где инструмент уместен.
- Show HN не перезаливать чаще раза в 2 недели; не запускаться в пт–вс.
- Не обещать фичи в комментариях — только roadmap в репо.
